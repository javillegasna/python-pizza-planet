...
def test_create(app, beverages, ingredients, size, client_data):
    (
        ...
        created_beverages,
        created_ingredients,
    ) = __create_sizes_and_items(ingredients, beverages, [size])
...
def test_get_all(app, beverages, ingredients, sizes, client_data):
    (
        ...
        created_beverages,
        created_ingredients,
    ) = __create_sizes_and_items(ingredients, beverages, sizes)
    created_orders = []
    for _ in range(5):
        order = __order(
            shuffle_list(created_beverages)[:3],
            shuffle_list(created_ingredients)[:3],
            get_random_choice(created_size),
            client_data,
        )
        created_order, _ = OrderController.create(order)
